...
import { Usermanagment } from '../../utils/data'
    ...
    return (
        <div>
            ...
            <Box className='row py-5'>
                <FilterTable title="کاربران" />
                <TableComponent TableHeader={Usermanagment}>
                    {rowscardNumber.map((row) => (
                        ...
                        >
                            ...
                            <TableCell sx={cellstyle}>{row.buy}</TableCell>
                            <TableCell sx={cellstyle}>{row.sell}</TableCell>
                            <TableCell sx={cellstyle}>{row.walletBalance}</TableCell>
                            <TableCell sx={cellstyle}>{row.walletCrypto}</TableCell>
                            <TableCell sx={cellstyle}>{row.Condition === "success" ? <span>فعال</span> : row.Condition === "unsuccess" ? <span>رد درخواست</span> : row.Condition === "defect" ? <span>نقص مدارک</span> : row.Condition === "waiting" ? <span>در انتظار احراز هویت</span> : <span>علت نامشخص</span>}</TableCell>
                            <TableCell sx={cellstyle}>
                                <Box dir="ltr">
                                    <Box>
                                        <IconButton
